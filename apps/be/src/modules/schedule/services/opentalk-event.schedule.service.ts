...
export class OpentalkEventScheduleService {
  ...
  async handleEventUpdate(eventId: number, newDate: string): Promise<{ before: Cycle[]; after: Cycle[] }> {
    this.logger.log(`Handling manual event update: Event ${eventId} -> ${newDate}`);
    ...
    const targetCycle = before.find(c => c.events.some(e => e.id === eventId));
    if (!targetCycle) {
        this.logger.warn(`Event ${eventId} not found in active/future cycles.`);
        return { before, after: [] };
    }

    this.logCyclesTable(`BEFORE - Update Event (${eventId} -> ${newDate})`, before);

    const holidays = await this.getHolidays();
    const changes = this.calculateEventUpdateChanges(before, eventId, newDate, holidays);

    const after = this.applyInMemoryChanges(before, changes);
    this.logCyclesTable(`AFTER - Update Event (${eventId} -> ${newDate})`, after);
    ...
    if (changes.eventsToUpdate.length > 0) {
      ...
      this.logger.log('No additional shifts needed (only target event updated).');
  ...
  private calculateEventUpdateChanges(
    ...
    const allEvents = cycles.flatMap(c => c.events.map(e => ({...e, cycleId: c.id})));
        ...
        if (prevDate) {
             ...
             if (event.date === prevDate) {
                 ...
                 if (event.date !== nextSatStr) {
                    changes.eventsToUpdate.push({ eventId: event.id, newDate: nextSatStr });
