...
export class FeedbackService {
  ...
  async sendFeedbackToChannel(username: string, feedbackDto: FeedbackDto) {
    ...
    if (feedbackDto.imageKey) {
      const presignUrl = await this.s3Service.getPresignedDownloadUrl(
        feedbackDto.imageKey,
        30000,
      );
      imageUrl = presignUrl.downloadUrl;
    }

    const message = this.formatFeedbackMessage(
      username,
      feedbackDto.text,
      imageUrl,
    );
    const channelId = WHITE_LIST_CHANNEL.DEV_FEEDBACK;
